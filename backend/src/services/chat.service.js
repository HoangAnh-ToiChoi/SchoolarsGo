...
const queryScholarships = async (filters) => {
  ...
  const sql = `SELECT ${cols} FROM scholarships WHERE ${conditions.join(' AND ')} ORDER BY deadline ASC LIMIT 8`;
...
const saveMessages = async (userId, userContent, assistantContent) => {
  ...
  try {
    await db.query(
      `INSERT INTO chat_messages (user_id, role, content) VALUES ($1, $2, $3), ($4, $5, $6)`,
      [userId, 'user', userContent, userId, 'assistant', assistantContent]
    );
  } catch {
